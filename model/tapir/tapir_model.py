...
from model.tapir import nets
from model.tapir import utils_tap
...
class Tapir(nn.Module):
  ...
  def forward(
      ...
      points_gt: torch.Tensor = None,
      visibs_gt: torch.Tensor = None
    ...
    # print(feature_grids.resolutions)
    # for i in range(len(feature_grids.lowres)):
    #   print(feature_grids.hires[i].shape, feature_grids.lowres[i].shape)
    ...
    # for i in range(len(query_features.lowres)):
    #   print(query_features.hires[i].shape, query_features.lowres[i].shape)
    ...
    if points_gt is not None:
      loss = sequence_loss(trajectories['tracks'], points_gt.permute(0, 2, 1, 3), visibs_gt.permute(0,2,1), visibs_gt.permute(0,2,1))
    else:
      loss = None
    return out, loss
  ...
  def get_query_features(
    ...
    for i, resolution in enumerate(resize_im_shape):
      if utils_tap.is_same_res(curr_resolution, resolution):
        query_feats.append(query_feats[-1])
        hires_query_feats.append(hires_query_feats[-1])
        continue
      position_in_grid = utils_tap.convert_grid_coordinates(
          query_points,
          shape[1:4],
          feature_grid[i].shape[1:4],
          coordinate_format='tyx',
      )
      position_in_grid_hires = utils_tap.convert_grid_coordinates(
          query_points,
          shape[1:4],
      ...
      interp_features = utils_tap.map_coordinates_3d(
          feature_grid[i], position_in_grid
      )
      hires_interp = utils_tap.map_coordinates_3d(
  ...
  def get_feature_grids(
    ...
    if refinement_resolutions is None:
      refinement_resolutions = utils_tap.generate_default_resolutions(
    ...
    for resolution in all_required_resolutions:
      ...
      if not utils_tap.is_same_res(curr_resolution, resolution):
        if utils_tap.is_same_res(curr_resolution, video.shape[-3:-1]):
          video_resize = video
        else:
          video_resize = utils_tap.bilinear(video, resolution)
  ...
  def estimate_trajectories(
    ...
    def train2orig(x):
      return utils_tap.convert_grid_coordinates(
          x,
    ...
    for ch in range(0, num_queries, query_chunk_size):
      ...
      if query_points_in_video is not None:
        ...
        infer_query_points = utils_tap.convert_grid_coordinates(
            infer_query_points,
  ...
  def refine_pips(
    ...
    for pyridx, (query, grid) in enumerate(zip(target_feature, pyramid)):
      ...
      coords = utils_tap.convert_grid_coordinates(
      ...
      neighborhood = utils_tap.map_coordinates_2d(grid, coords2)
    ...
    x = utils_tap.einshape('bnfc->(bn)fc', mlp_input)
    res = self.torch_pips_mixer(x.float())
    res = utils_tap.einshape('(bn)fc->bnfc', res, b=mlp_input.shape[0])

    pos_update = utils_tap.convert_grid_coordinates(
        res[..., :2],#.detach(),
        (resized_w, resized_h),
        (orig_w, orig_h),
    )
  ...
  def tracks_from_cost_volume(
    ...
    cost_volume = utils_tap.einshape('tbnhw->(tbn)hw1', cost_volume)
    ...
    pos_rshp = utils_tap.einshape('(tb)hw1->t(b)hw1', pos, t=shape[0])
    pos = utils_tap.einshape(
    ...
    points = utils_tap.heatmaps_to_points(pos, im_shp, query_points=query_points)
    ...
    expected_dist = utils_tap.einshape(
        '(tbn)1->bnt', occlusion[..., 1:2], n=shape[2], t=shape[0]
    )
    occlusion = utils_tap.einshape(
